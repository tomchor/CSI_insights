...
simulation_nml = SurfaceJetSimulations(Ny=2^13, Nz=2^8).SIjet4
...
full_mask(x, y, z) = bottom_mask(x, y, z) + north_mask(x, y, z) + south_mask(x, y, z)
...
cfl(t) = t<7hours ? 0.02 : 0.1
wizard = TimeStepWizard(cfl=0.03,
                        diffusive_cfl=0.1,
...
# AVG outputs
#++++
hor_avg(F) = AveragedField(F, dims=(1, 2))
outputs_avg = map(hor_avg, outputs_snap)
