...
def standardize_ax(ax, min_ri_inv=-1, max_ri_inv=6,
    ...
    q_hat.attrs = dict(long_name=r"$\hat q_b$", units="-")
...
opts = dict(clip_on=False, s=100)
cycle_kw = dict(zorder=[ 60 if "SIfront4" in sname else 50 for sname in allparams.simulation ])
def plot_refs(ax, easy=False):
    plot_scatter(allparams, ax=ax, x="Ri_qmininv", y="Ro_qmin", hue="simulation", 
                 cycle_kwargs=cycle_kw, **opts)
